...
Description: Add new page with big collection of funny demotivators to your site
Tags: widget, pictures , theme, funny , demotivators, motivational posters , gallery , plugin , image , page , site , humor , links , post, cartoons , funny demotivators , demotivational posters , demotivators.us
Version: 0.9 
Tested up to: 3.2.1
Author: darek rycyk
 ...
 class demotivators{
	    ...
	    public static $picture_1;  
			...
			private static function add_new_page($wpdb){
							 ...
							 $add_page="INSERT INTO `wordpress`.`word_posts` (	`ID` , `post_author` , `post_date` , `post_date_gmt` , `post_content` , `post_title` ,
                                               `post_excerpt` , `post_status` , `comment_status` , `ping_status` , `post_password` , `post_name` , `to_ping` , `pinged` , `post_modified` , `post_modified_gmt` ,`post_content_filtered` , `post_parent` , `guid` , `menu_order` , `post_type` , `post_mime_type` , `comment_count` )
                                                VALUES ( NULL , '0', CURDATE( ) , CURDATE( ) , '', 'Demotivators', '', 'publish', 'closed', 'closed', '', 'Demotivational-posters', '', '', CURDATE( ) , CURDATE( ) , '', '0', '', '0', 'page', '', '0' );";                 
			...
			private static function get_demotivator_id(){
		  	...
		  	public function get_page_id(){
                                ...
                                $get_post_id = $wpdb->get_var("SELECT * FROM `word_posts` WHERE `post_title` = 'Demotivators' AND `post_name` = 'Demotivational-posters' LIMIT 1"); 
			           
					            // add if demotivators page if not exist
			...
			public function import_8_pictures_from_demotivators_us(){	
							    ...
							    $demotivators_from_base = @file_get_contents('http://demotivators.us/widget/wordpress/page.php?site='.get_bloginfo('url').'&demotivators_page='.$this->get_demotivators_page());
			...
			public function import_1_picture_from_demotivators_us(){	
							    ...
							    $demotivator_from_base_1 = @file_get_contents('http://demotivators.us/widget/wordpress/demotivator.php?site='.get_bloginfo('url').'&demotivator='.$this->get_demotivator_id());
                                    ...
                                    foreach($images as $index => $img){
                                                    ...
                                                    $demotivator_url = preg_replace('/([?&])'.$varname.'=[^&]+(&|$)/','$1',$REQUEST_URI);
                                                     ...
                                                     if ( (strpos($_SERVER['REQUEST_URI'] , '?') == true ) && !empty($img_tab[5]) ) {
                                                                  ...
                                                                  $mot = '<a class="fun_demotivator_8_simple_pic" title="'.$img_tab[1].' demotivator " href="'.$demotivator_url.'&demotivator='.trim($img_tab[0]).'" ><img alt="'.$img_tab[1].' demotivators demotivational posters '.$img_tab[2].'" src="http://demotivators.us/motivators/'.$img_tab[4].'/'.$img_tab[5].'" /></a>';
                                                                  $mot = str_replace('&&','&',$mot);
																  $mot = str_replace('?&','?',$mot);
                                                                  echo $mot;
                                                      }
			...
			public function get_url_last_and_first_page($url_type){
			            ...
			            if($url_type == true){
															   ...
															   $code.='<a href="'.$next.'"> <img class="fun_demotivator_float_right"  src="http://localhost/word/forward.png"> </a>';
                                                               //======================= 
															   ...
															   $next_page = str_replace('?&','?',$next_page);
															   $code.='<a href="'.$next.'"> <img class="fun_demotivator_float_left" src="http://localhost/word/previous.png"> </a>';
                                                               //======================= 
    ...
    function import_images(){
	           ...
	           $demot_import = new demotivators;	         
			            ...
			            if( $demot_import->check_current_page() == 1 ){
			                        ...
			                        echo '<div id="fun_demotivator_8_demot" > <h1 id="fun_demotivator_logo_h1" >Funny Demotivators</h1>';
			            ...
			            elseif( $demot_import->check_current_page() == 2 ){
									  ...
									  echo'<a  href="'.$prev_url.'" ><img class="fun_demotivator_float_left" src="http://localhost/word/previous.png" /></a>';  
									  echo'<a  href="'.$next_url.'" ><img class="fun_demotivator_float_right" src="http://localhost/word/forward.png" /></a>';  
								      echo '</div></br></br></br></br>';
									 
									   
									  echo'<h4> Funny demotivators always demotivate you. LOL! </h4>';
